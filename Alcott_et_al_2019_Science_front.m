...
function run = Alcott_et_al_2019_Science_front(S)
...
pars.korgP_prox = 1 ;
%New
% OP_P_Burial_0 = starting.POC_P_Burial_0 / 250 ;
% pars.kOP_P_0 = OP_P_Burial_0 / ( starting.Prox_Prod_Photo * ( ( ( 1-starting.fanoxic ) / 250 ) + ( starting.fanoxic / 1100 ) ) ) ;
%Old
...
% pars.kCaP_prox = P_AuthP_P_0 / ( pars.kPrel_prox * starting.OP_P ) ; OLD
...
%     
%     %%%% carb C cycle values from COPSE
%     pars.Fccdeg_0 = 15e12 + 1.25e12 ; 
%     pars.Fcarbw_0 = 8e12 ;
    pars.Fccdeg_0 = 25e12 ; 
    pars.Fcarbw_0 = 16e12 ;
    pars.Fsfw_0 = 1.75e12 ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% pars.Foxidw_0 = pars.O2_A_Weathering ; %Oxidative weathering
pars.Focdeg_0 = 0 ; % Assume this for now - Organic carbon degassing
% pars.Fcarbw_0 = 8e12 ; %Carbonate weathering flux
% pars.Fsilw_0 = 14e12 ; %Silicate weathering flux
% pars.Fmccb_0 = pars.Fcarbw_0 + pars.Fsilw_0 ; %Carbonate carbon burial
basfrac = 0.3 ;
    pars.k_granw = pars.Fsilw_0 * (1-basfrac) ;
    pars.k_basw = pars.Fsilw_0 * basfrac ;
%  
% % % Calculate carbonate degassing and sea floor weathering rates at present
% %day
% pars.inorg_difference = pars.O2_A_Weathering + pars.rgf - pars.Fmocb_0 - pars.Flocb_0 + pars.Fcarbw_0 - pars.Fmccb_0 + pars.Focdeg_0;
% pars.Fccdeg_0 = (pars.inorg_difference * (11/10))*-1 ;
% pars.Fsfw_0 = (pars.inorg_difference * (1/10))*-1 ;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%   Sensitivity analysis   %%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    %%% MONTY PYTHON MULTIPLE RUNS
...
% %         sensparams.D = 1 + 0.9*randminusplus1 ; %Use this, it's fairer
%         sensparams.D = 0.5 + rand*(2-0.5); %Test
%         sensparams.fbiota =  0.15 + rand*(1-0.15);
% % sensparams.fbiota = 1+ (0.2*randminusplus3) ;
% %         sensparams.C = 1 + 0.5*randminusplus3 ;
%         shallowsteep = rand ; %define if curve is concave or convex
% %         if shallowsteep <=0.5
% %             sensparams.C = (rand /(5/2))+0.5 ; %Number between 0.5 and 1 ;
% %         else
% %             sensparams.C = 1/((rand /(5/2))+0.5);
% %         end
%         
% %         if shallowsteep <=0.5
% %             sensparams.C = (rand /(5/2))+0.33 ; 
% %         else
% %             sensparams.C = 1/((rand /(5/2))+0.33);
% %         end
%         sensparams.C = 0.33 + rand*(3-0.33) ; 
% 
% 
%         
...
% %         sensparams.locb = rand ;

%%% MEAN RUN

        %%%%%%% parameter space to test
        sensparams.D = 0.25 + rand*(2-0.25); %Test
%         sensparams.D = 0.5 + 1.25 ;  %Middle number
        sensparams.fbiota =  0.15 + rand*(1-0.15);
%         sensparams.fbiota =  0.575 ; %
        sensparams.C = 0.25 + rand*(3-0.25) ; 
%         sensparams.C = 0.33 + 0.67 ; %Equals one to just use Hayes and Waldbauer curve.


        
        sensparams.CP = pars.CPoxic + rand*(1100-pars.CPoxic) ; 
%         sensparams.CP = 603 ; %Middle Number
        sensparams.EXP = 0.1 + rand* (0.75-0.1) ;
%         sensparams.EXP = 0.1 + 0.425 ;%Middle number
        sensparams.EXP2 = sensparams.EXP + rand* (1-sensparams.EXP) ;
%         sensparams.EXP2 = 1 ;%Needs to equal 1
        sensparams.EXPtiming = 1.7 + rand*(3-1.7);
%         sensparams.EXPtiming = 2.35;%Middle number
...
% %P 
% subplot(4,3,2)
% semilogy(state.time, (state.SRP_P + state.SRP_D + state.SRP_S + state.SRP_DP) ) ;
% hold on
% semilogy(state.time, state.SRP_P)
% hold on
% semilogy(state.time, state.SRP_D)
% hold on
% semilogy(state.time, state.SRP_S)
% hold on
% semilogy(state.time, state.SRP_DP)
% title('Soluble Reactive Phosphorus')
% ylabel('mol')
% xlabel('Time')
% legend('Ptot','Prox','Dist','Surf','Deep')
% xlim([-4e9 0])
% 
% %Shelf P burial
% subplot(4,3,3)
% plot(state.time, (state.Dist_Preac_Burial + state.Prox_Preac_Burial + state.Deep_Preac_Burial )) ;
% hold on
% plot(state.time, state.River_SRP) ;
% % hold on
% % plot(state.time, state.Fe_SCAV_SURF) ;
% hold on
% plot(state.time, state.Prox_Preac_Burial)
% hold on
% plot(state.time, state.Dist_Preac_Burial)
% hold on
% hold on
% plot(state.time, state.Deep_Preac_Burial)
% title('P Burial')
% ylabel('molyr-1')
% xlabel('Time')
% 
% legend('P Burial', 'P weathering', 'SCAV','Prox','Dist','Deep')
% xlim([-4e9 0])
% 
% %Fraction of anoxia on the shelf and proximal waters
% subplot(4,3,4)
% plot(state.time, state.fanoxicprox) ;
% hold on
% plot(state.time, state.fanoxicdist) ;
% title('fraction anoxic')
...
% subplot(4,3,5)
% plot(state.time, (state.GAST-273.15)) ;
% title('Temp')
% ylabel('degrees C')
% xlabel('Time')
% xlim([-4e9 0])
...
% subplot(4,3,6)
% plot(state.time, (state.PP_P + state.PP_D + state.PP_S)) ;
% % hold on
% % plot(state.time, (state.POC_Min_P + state.POC_Min_D + state.POC_Min_S + state.POC_DP_Resp)) ;
% title('Total primary production')
% ylabel('molyr-1')
% xlabel('Time')
% legend('Total PP')
% xlim([-4e9 0])
...
% %CO2 ppm
% subplot(4,3,7)
% semilogy(state.time, (state.CO2atm*1e6)) ;
% title('CO2')
% ylabel('ppm')
% xlabel('Time')
% xlim([-4e9 0])
...
% 
% subplot(4,3,11)
% semilogy(state.time, state.fbiota)
% hold on
% semilogy(state.time, state.D)
% hold on
% semilogy(state.time, state.PG)
% hold on
% semilogy(state.time, state.C)
% hold on
% semilogy(state.time, state.U)
% hold on
% semilogy(state.time, state.EXPOSED)
% title('forcings')
% legend('fbiota','degassing','PG','C','U','landfrac')
% xlim([-4e9 0])
% 
% subplot(4,3,12)
% semilogy(state.time, state.Fcarbw)
% hold on
% semilogy(state.time, state.Fgranw)
% hold on
% semilogy(state.time, state.Fbasw)
% hold on
% semilogy(state.time, state.Fsilw)
% hold on
% semilogy(state.time, state.Foxidw)
% hold on
% semilogy(state.time, state.Fsfw)
% title('weathering')
% legend('carbw','granw','basw','silw','oxidw','sfw')
% xlim([-4e9 0])
% 
% 
% figure
% plot(state.time, state.doxidw)
% hold on
% plot(state.time, state.docdeg)
% hold on
% plot(state.time, state.dcarbw)
% hold on
% plot(state.time, state.dccdeg)
% hold on
% plot(state.time, state.drgf)
% hold on
% plot(state.time, state.dmocb)
% hold on
% plot(state.time, state.dmccb)
% hold on
% plot(state.time, state.dsfw)
% hold on
% plot(state.time, state.dlocb)
% title('carbon isotope fluxes')
% legend('oxidw','ocdeg','carbw','ccdeg','rgf','mocb','mccb','sfw','locb')
% xlim([-4e9 0])
% 
% 
% 
